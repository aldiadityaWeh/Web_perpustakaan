            ...
            @if ($bukus->hasPages())
                <div class="flex items-center justify-between w-full">
                    <div class="hidden sm:block text-sm text-gray-500">
                        Menampilkan halaman <span class="font-bold text-gray-700">{{ $bukus->currentPage() }}</span> dari <span class="font-bold text-gray-700">{{ $bukus->lastPage() }}</span>
                    </div>
                    ...
                    <nav class="relative z-0 inline-flex rounded-lg shadow-sm -space-x-px" aria-label="Pagination">
                        ...
                        @else
                            <a href="{{ $bukus->previousPageUrl() }}" class="relative inline-flex items-center px-3 py-2 rounded-l-lg border border-gray-200 bg-white text-gray-600 hover:bg-purple-50 hover:text-purple-600 transition-colors">
                                <i class="ph ph-caret-left text-sm"></i>
                            </a>
                        @endif
                        ...
                        @php
                            $start = max($bukus->currentPage() - 1, 1);
                            $end = min($start + 2, $bukus->lastPage());
                        ...
                        @for ($i = $start; $i <= $end; $i++)
                            ...
                            @else
                                <a href="{{ $bukus->url($i) }}" class="relative inline-flex items-center px-4 py-2 border border-gray-200 bg-white text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                                    {{ $i }}
                                </a>
                            @endif
                        @endfor
                        ...
                        @if ($bukus->hasMorePages())
                            <a href="{{ $bukus->nextPageUrl() }}" class="relative inline-flex items-center px-3 py-2 rounded-r-lg border border-gray-200 bg-white text-gray-600 hover:bg-purple-50 hover:text-purple-600 transition-colors">
